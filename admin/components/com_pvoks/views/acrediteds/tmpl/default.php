...
?>
<h2><i class="icon-next"></i>&nbsp;<?php echo $this->title; ?></h2>
...
<form action="<?php echo JRoute::_('index.php?option=com_pvoks&view=categories'); ?>" method="get" name="adminForm" id="adminForm">
    <div class="filterForm">
	   ...
	   <select name="filter_acredited_id">
		 ...
		 <?php 
		 $db->setQuery('select u.id,u.name 
		 from #__users u, #__pvoks_members m
		 where m.user_id = u.id and m.state = 3
		 order by name');
		 $res = $db->loadObjectList();
		 foreach ($res as $res1) : 
		 ?>
	     <option value="<?php echo $res1->id; ?>"<?php if($input->get('filter_acredited_id',0)==$res1->id) echo ' selected="selected"'; ?>>
		     <?php echo $res1->name; ?>
		 </option>
		 <?php endforeach; ?>
	   </select>
	...
	<table class="table table-striped" id="articleList" class="adminList" style="width:100%">
		<thead>
			<tr>
				...
				<th class="nowrap left">
					<?php $pvoksHelper->echoColHeader('PVOKS_ACREDITEDS_NAME','u.name', $listDirn, $listOrder); ?>
				</th>
				<th>
					<?php $pvoksHelper->echoColHeader('PVOKS_ACREDITEDS_ACREDITED_NAME','b.name', $listDirn, $listOrder); ?>
				</th>
				<th>
					<?php $pvoksHelper->echoColHeader('PVOKS_ACREDITED_TERMINATE','a.terminate', $listDirn, $listOrder); ?>
				</th>
			</tr>
		</thead>
		...
		?>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td class="left"><?php echo $this->escape($item->acredited); ?></td>
				<td class="left"><?php echo $this->escape($item->terminate); ?></td>
			</tr>
		<?php endforeach ?>
		</tbody>
		<tfoot>
	...
	<input type="hidden" name="view" value="acrediteds" />	
