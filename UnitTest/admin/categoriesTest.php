...
class pvoksTestCategories extends PHPUnit_Framework_TestCase {
	...
	function __construct() {
		...
		$viewName = 'pvoks';
		parent::__construct();
		$this->controller = new PvoksControllerCategories();
	}
    ...
    public function test_categories_save4()  {
		global $testData;
		// modositás jó adatokkal
		$testData->clear();
		$testData->setInput('jform',array("id" => 1, "title" => "testTitle1", "category_type" => 1));
		$testData->addDbResult(array()); // alias duplicate check
		$this->controller->save();
		$this->expectOutputRegex('/SAVED/');
    }
